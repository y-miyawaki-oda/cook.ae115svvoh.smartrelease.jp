...
*/
		elseif(is_page('about/guide')) {
			return '調理師国家資格を取得する東京の専門学校' . ' | ' . get_bloginfo();
		}
		elseif(is_page()) {
	...
	function customize_description($description) {
		...
		elseif(is_post_type_archive("column")) {
			$description = 'ファッションの専門学校進学を考える上で参考になる話題について書き記していきます。';
		}
		...
		elseif(is_post_type_archive("campusreport") || is_singular("campusreport")) {
			$description = '服飾、ファッションについて学べる専門学校。東京都中野区の織田調理師専門学校でスタイリスト、デザイナー、パタンナー、アパレル、ファッションビジネスのプロを目指そう！なりたい自分になって、夢をかなえよう。';
		}
	...
	function customize_keywords($keywords) {
		if(is_post_type_archive("news") || is_tax("cat_news")) {
			$keywords = '調理師,専門学校,東京,新着情報,ニュース,織田';
		}
		elseif(is_front_page()) {
			$keywords = '織田,調理師,専門学校,国家資格';
		}
		elseif(is_post_type_archive("talk")) {
			$keywords = '織田,調理師,専門学校,国家資格';
		}
		...
		elseif(is_singular("coordinatecontest")) {
			$keywords = '調理,アパレル,専門学校,スタイリスト,デザイナー,マーチャンダイザー,ファッションアドバイザー';
		}
		elseif(is_post_type_archive("report") || is_singular("report")) {
			$keywords = '織田,調理師,専門学校,国家資格';
		}
		return $keywords;
	}
...
<section class="sec_oc is_load_anime fadein">
	<div class="wrap">
		<div class="box_white_r">
			<h2 class="ttl_40 raleway nunito">NEXT OPEN CAMPUS</h2>
...
EOT;
			foreach($posts as $post) {
				...
				$title = esc_html(get_the_title());
				$image = get_the_post_thumbnail(get_the_ID(), "opencampus_list3", array("alt" => "", "class" => "cut_img cover"));
...
<section id="oc" class="sec_oc2 mt70 is_anime fadein">
	<div class="wrap w1200">
		<div class="box_white_r">
			<h2 class="ttl_sec"><span class="en raleway nunito">OPEN CAMPUS</span>オープンキャンパス</h2>
			<p class="lead">織田調理師専門学校のオープンキャンパスでは<br class="sp">充実した授業体験を用意しています。</p>
			...
			$html .= <<<EOT
						<div class="block">
							...
							<div class="main">
								<table>
									<tr>
										<th class="bc nunito">Sun</th>
										<th class="bc nunito">Mon</th>
										<th class="bc nunito">Tue</th>
										<th class="bc nunito">Wed</th>
										<th class="bc nunito">Thu</th>
										<th class="bc nunito">Fri</th>
										<th class="bc nunito">Sat</th>
									</tr>
									<tr>
EOT;
				...
				if($val["class"] !== "prev" && $val["class"] !== "next") {
					...
					if($oc_arr[$date_txt]["bread"] || $oc_arr[$date_txt]["group"] || $oc_arr[$date_txt]["chinese"] || $oc_arr[$date_txt]["japanese"] || $oc_arr[$date_txt]["western"] || $oc_arr[$date_txt]["confectionery"] || $oc_arr[$date_txt]["special"]) {
						...
						if($oc_arr[$date_txt]["bread"]) {
							$table .= '<li><img src="'.esc_url(get_template_directory_uri()).'/img/index/oc_icon_stylist_business.png" alt=""></li>';
						}
						if($oc_arr[$date_txt]["group"]) {
							$table .= '<li><img src="'.esc_url(get_template_directory_uri()).'/img/index/group_lunch.png" alt=""></li>';
						}
						if($oc_arr[$date_txt]["confectionery"]) {
							$table .= '<li><img src="'.esc_url(get_template_directory_uri()).'/img/index/oc_icon_design_technical.png" alt=""></li>';
						}
						if($oc_arr[$date_txt]["chinese"]) {
							$table .= '<li><img src="'.esc_url(get_template_directory_uri()).'/img/index/chinese_cook.png" alt=""></li>';
						}
						if($oc_arr[$date_txt]["japanese"]) {
							$table .= '<li><img src="'.esc_url(get_template_directory_uri()).'/img/index/japanese_cook.png" alt=""></li>';
						}
						if($oc_arr[$date_txt]["western"]) {
							$table .= '<li><img src="'.esc_url(get_template_directory_uri()).'/img/index/western_cook.png" alt=""></li>';
						}
						if($oc_arr[$date_txt]["special"]) {
							$table .= '<li><img src="'.esc_url(get_template_directory_uri()).'/img/index/special.png" alt=""></li>';
						}
						$table .= '</ul>';
					}
				}
		...
		$html .= <<<EOT
				...
				<div class="course grid2">
					<div class="item-icon">
						<p class="icon"><img src="{$tmp_dir}/img/index/western_cook.png" alt=""></p>
						<p class="txt">西洋料理実習</p>
					</div>
					<div class="item-icon">
						<p class="icon"><img src="{$tmp_dir}/img/index/oc_icon_design_technical.png" alt=""></p>
						<p class="txt">製菓実習</p>
					</div>
					<div class="item-icon" style="margin-top: auto;">
						<p class="icon"><img src="{$tmp_dir}/img/index/japanese_cook.png" alt=""></p>
						<p class="txt">日本料理実習</p>
					</div>
					<div class="item-icon">
						<p class="icon"><img src="{$tmp_dir}/img/index/chinese_cook.png" alt=""></p>
						<p class="txt">中国料理実習</p>
					</div>
					<div class="item-icon">
						<p class="icon"><img src="{$tmp_dir}/img/index/oc_icon_stylist_business.png" alt=""></p>
						<p class="txt">製パン実習</p>
					</div>
					<div class="item-icon">
						<p class="icon"><img src="{$tmp_dir}/img/index/group_lunch.png" alt=""></p>
						<p class="txt">給食実習</p>
					</div>
					<div class="item-icon">
						<p class="icon"><img src="{$tmp_dir}/img/index/special.png" alt=""></p>
...
EOT;
		...
		$html .= <<<EOT
					...
					<p class="btn tac"><a href="./oc/" class="btn_black fade raleway nunito">read more</a></p>
				</div>
			</div>
		</div>
	</div>
...
EOT;
	...
	function shortcode_top_campusreport() {
		...
		global $post;
		$campusreport_link = esc_url(get_post_type_archive_link("talk"));
		$args = array(
			'posts_per_page' => 4,
			'post_type' => 'talk',//campusreport
			//'orderby' => 'menu_order',
			//'order' => 'ASC'
			'orderby' => 'date',
			'order' => 'DESC'
...
<section class="sec_column mt70 is_anime fadein">
	<div class="wrap w1200">
		<div class="box_white_r">
			<h2 class="ttl_sec"><span class="en raleway nunito">DIARY</span>実習ダイアリー</h2>
...
EOT;
			...
			<p class="btn tac"><a href="{$campusreport_link}" class="btn_black fade raleway nunito">read more</a></p>
		</div>
	</div>
</section>
<!-- /.sec_column -->
...
<section class="sec_news mt70 is_anime fadein">
	<div class="wrap w1200">
		<div class="box_white_r">
			<h2 class="ttl_sec"><span class="en raleway nunito">NEWS</span>ニュースリリース</h2>
			<ul class="list">
EOT;
			foreach($posts as $post) {
				setup_postdata($post);
...
EOT;
			...
			<p class="btn tac"><a href="{$news_link}" class="btn_black fade raleway nunito">read more</a></p>
		</div>
	</div>
</section>
<!-- /.sec_news -->
EOT;
	...
	function set_kv_image() {
		...
		if(is_front_page()) {
			...
			$html = <<<EOT
<img src="{$image_pc}" alt="すべてを学ぶから目指すべき『道』が見える。" class="pc">
<img src="{$image_sp}" alt="すべてを学ぶから目指すべき『道』が見える。" class="sp">
EOT;
		}
		else {
